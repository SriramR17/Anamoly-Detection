...
def plot_and_save_results(scores, focus_metric='accuracy', output_dir=None):
    """
    Combined function to create visualization and save algorithm comparison results.
    ...
    try:
        ...
        for model_name, score_dict in scores.items():
            data.append({
                ...
                'Accuracy_Mean': score_dict['cv_accuracy_mean'],
                'Accuracy_Std': score_dict['cv_accuracy_std'],
                'F1_Mean': score_dict['cv_f1_mean'],
                'F1_Std': score_dict['cv_f1_std'],
                'Precision_Mean': score_dict['cv_precision_mean'],
                'Precision_Std': score_dict['cv_precision_std'],
                'Recall_Mean': score_dict['cv_recall_mean'],
                'Recall_Std': score_dict['cv_recall_std'],
...
def get_or_train_model(X_train, y_train, model_path=None, force_retrain=False, save_results=True):
    """
    ...
    Args:
        ...
        save_results: If True, save plots and CSV results when training
    ...
    # Save results and create visualization if requested
    if save_results and 'scores' in results:
        print("\n📊 Generating results visualization and saving data...")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if os.path.exists(MODEL_PATH):
            ...
            # Train models with visualization and saving
            results = get_or_train_model(X_train, y_train, MODEL_PATH, save_results=True)
            best_model = results['best_model']
