...
const evaluationMetrics = [
  { name: 'Accuracy', value: 98.93, deviation: 0.15, color: '#10B981', icon: Target },
  { name: 'F1-Score', value: 98.03, deviation: 0.29, color: '#8B5CF6', icon: BarChart3 },
  { name: 'Precision', value: 99.51, deviation: 0.13, color: '#3B82F6', icon: Search },
  { name: 'Recall', value: 96.59, deviation: 0.53, color: '#F59E0B', icon: Eye }
];
...
// Top Anomaly Networks Data
const topAnomalyNetworks = [
  { 
    name: 'Network-Alpha-001', 
    totalSamples: 125847, 
    anomaliesDetected: 2341, 
    anomalyRate: 1.86, 
    lastUpdate: '2 min ago', 
    recentAnomaly: 'Port Scan Attack',
    severity: 'high'
  },
  { 
    name: 'Network-Beta-002', 
    totalSamples: 98563, 
    anomaliesDetected: 1897, 
    anomalyRate: 1.92, 
    lastUpdate: '5 min ago', 
    recentAnomaly: 'DDoS Attempt',
    severity: 'high'
  },
  { 
    name: 'Network-Gamma-003', 
    totalSamples: 87234, 
    anomaliesDetected: 1654, 
    anomalyRate: 1.90, 
    lastUpdate: '8 min ago', 
    recentAnomaly: 'Malware Traffic',
    severity: 'medium'
  },
  { 
    name: 'Network-Delta-004', 
    totalSamples: 76895, 
    anomaliesDetected: 1423, 
    anomalyRate: 1.85, 
    lastUpdate: '12 min ago', 
    recentAnomaly: 'Brute Force',
    severity: 'medium'
  },
  { 
    name: 'Network-Epsilon-005', 
    totalSamples: 65432, 
    anomaliesDetected: 1198, 
    anomalyRate: 1.83, 
    lastUpdate: '15 min ago', 
    recentAnomaly: 'SQL Injection',
    severity: 'low'
  },
  { 
    name: 'Network-Zeta-006', 
    totalSamples: 54321, 
    anomaliesDetected: 987, 
    anomalyRate: 1.82, 
    lastUpdate: '18 min ago', 
    recentAnomaly: 'XSS Attack',
    severity: 'low'
  }
];

// Anomaly vs Normal Distribution Data
...
// Interactive Horizontal Bar Chart for Evaluation Metrics
const EvaluationMetrics = ({ metrics, selectedMetric, setSelectedMetric }) => {
          ...
          return (
            <div 
              ...
              className={`group cursor-pointer p-4 rounded-lg transition-all duration-300 ${
                isSelected ? 'bg-gray-700 ring-2 ring-blue-500' : 'hover:bg-gray-750'
              }`}
...
// Algorithm Performance Line Chart
const AlgorithmComparison = ({ data, selectedAlgorithm, setSelectedAlgorithm }) => {
        ...
        <div className="mt-4 p-4 bg-gray-700 rounded-lg animate-fadeIn">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              ...
              <span className="text-blue-400 font-medium ml-2">{data[selectedAlgorithm].accuracy}%</span>
            </div>
            <div>
              <span className="text-gray-400">F1-Score:</span>
              <span className="text-green-400 font-medium ml-2">{data[selectedAlgorithm].f1}%</span>
            </div>
            <div>
              <span className="text-gray-400">Precision:</span>
              <span className="text-purple-400 font-medium ml-2">{data[selectedAlgorithm].precision}%</span>
            </div>
            <div>
              <span className="text-gray-400">Recall:</span>
              <span className="text-orange-400 font-medium ml-2">{data[selectedAlgorithm].recall}%</span>
            </div>
          </div>
        </div>
      )}
...
// Peak Hours Speedometer Component
const PeakHoursSpeedometer = () => {
  const [gaugeValue, setGaugeValue] = useState(0);
  const peakHour = 14; // 2 PM peak hour
...
// Network Tile Component
const NetworkTile = ({ network, index, isSelected, onClick }) => {
      ...
      <div className="space-y-3">
        ...
        <div className="pt-3 border-t border-gray-700">
          ...
          <div>
            ...
            <span className={`text-xs font-medium ${severityTextColors[network.severity]}`}>
              {network.recentAnomaly}
            </span>
          </div>
        </div>
        
        {/* Anomaly Rate Progress Bar */}
        <div className="mt-3">
          <div className="w-full bg-gray-700 rounded-full h-2">
            <div 
              ...
              style={{
                ...
                backgroundColor: network.severity === 'high' ? '#EF4444' : network.severity === 'medium' ? '#F59E0B' : '#10B981'
              }}
            />
          </div>
        </div>
...
const TopNetworks = ({ networks, selectedNetwork, setSelectedNetwork }) => (
  <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
    <h3 className="text-xl font-semibold text-white mb-6 flex items-center">
              ...
              <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-gray-700 rounded-lg p-4">
                    <div className="text-gray-400 text-sm mb-1">Total Samples</div>
                    <div className="text-2xl font-bold text-white">{dashboardData.total_samples.toLocaleString()}</div>
                  </div>
                  <div className="bg-gray-700 rounded-lg p-4">
                    <div className="text-gray-400 text-sm mb-1">Anomalies Detected</div>
                    <div className="text-2xl font-bold text-red-400">{dashboardData.total_anomalies.toLocaleString()}</div>
                  </div>
                  <div className="bg-gray-700 rounded-lg p-4">
                    <div className="text-gray-400 text-sm mb-1">Anomaly Rate</div>
                    <div className="text-2xl font-bold text-orange-400">{dashboardData.anomaly_rate}%</div>
                  </div>
                  <div className="bg-gray-700 rounded-lg p-4">
                    <div className="text-gray-400 text-sm mb-1">Last Updated</div>
                    <div className="text-sm text-gray-300">{dashboardData.last_updated}</div>
                  </div>
                </div>
                
                {/* Real Distribution Data */}
                {dashboardData.total_samples > 0 && (
                  <div className="mt-6">
                    <h4 className="text-lg font-semibold text-white mb-4">Actual Distribution</h4>
                    <ResponsiveContainer width="100%" height={300}>
                      <PieChart>
                        <Pie
                          data={[
                            { name: 'Normal', value: dashboardData.total_samples - dashboardData.total_anomalies, color: '#10B981' },
                            { name: 'Anomalies', value: dashboardData.total_anomalies, color: '#EF4444' }
                          ]}
                          cx="50%"
                          cy="50%"
                          outerRadius={100}
